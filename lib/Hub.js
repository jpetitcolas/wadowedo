...
module.exports = function(eventEngine) {
    ...
    hub.sendInitialData = function(player) {
        ...
        player.socket.emit('update:allResources', player.tribe.resources);
        player.socket.emit('update:allBuildings', player.tribe.buildings);
        player.socket.emit('update:allSkills', player.skills);
        player.socket.emit('update:allTechnologies', player.tribe.technologies);
        player.socket.emit('tribeList', hub.getTribesWithPlayerNames());
        player.socket.emit('update:allClickCount', player.tribe.getAllCraftingClicks());
        player.socket.emit('update:health', player.tribe.health);
    ...
    hub.addEventListeners = function(player) {
        ...
        player.socket.on('crafting', function(objectName) {
            player.craft(require('../models/technologies/' + objectName));
        });

        player.socket.on('build', function(objectName) {
            player.build(require('../models/buildings/' + objectName));
        });
        ...
        // Handle events
        player.socket.on('event:vote', function(data) {
            eventEngine.vote(data);
        });
    };
